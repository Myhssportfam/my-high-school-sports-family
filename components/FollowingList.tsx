...
import { fetchFollowingPaginated, fetchUserById, searchFollowersServer } from '../lib/users'
import Link from 'next/link'

export default function FollowingList({ userId, pageSize = 12, maxPrefetchPages = 5 }: { userId: string; pageSize?: number; maxPrefetchPages?: number }) {
      ...
      try {
        if (debouncedFilter) {
          ...
          for (let i = 0; i < maxPrefetchPages; i++) {
            const res = await fetchFollowingPaginated(userId, pageSize, cursor)
            if (!mounted) return
          ...
          const res = await fetchFollowingPaginated(userId, pageSize)
          if (!mounted) return
          const profiles = await Promise.all(res.map(async (r) => {
            const p = await fetchUserById(r.id)
            return { id: r.id, profile: p, createdAt: r.createdAt }
  ...
  async function loadMore() {
    ...
    setLoadingMore(true)
    try {
      const res = await fetchFollowingPaginated(userId, pageSize, lastId)
      const profiles = await Promise.all(res.map(async (r) => {
        const p = await fetchUserById(r.id)
        return { id: r.id, profile: p, createdAt: r.createdAt }
      }))
  ...
  if (!items || items.length === 0) return (
    <div className="p-4">
      <input value={filter} onChange={(e) => setFilter(e.target.value)} placeholder="Search following" className="w-full p-2 border rounded mb-2" />
